  ...
  return <div className="shell">
    ...
    <main className="main">
      <header className="topbar">
        ...
        <form onSubmit={search} style={{flex:1}}><input aria-label="Search LEADORA" className="search" value={globalSearch} onChange={e=>setGlobalSearch(e.target.value)} placeholder="Search pages, prospects and contacts…" /></form>
...
function OutreachPage({businesses,prospects,setProspects,drafts,setDrafts,outreach,setOutreach}:{businesses:BusinessProfile[];prospects:Prospect[];setProspects:(v:Prospect[])=>void;drafts:Draft[];setDrafts:(v:Draft[])=>void;outreach:Outreach[];setOutreach:(v:Outreach[])=>void}) {
  ...
  async function send(id:string){
    ...
    try {
      ...
      const response=await fetch('/api/gmail/send',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({from,to:p.email,subject:cleanSubject,body:cleanBody})});
  ...
  {sendError&&<div role="alert" className="card" style={{color:'#b42318',marginBottom:16}}>{sendError}</div>}<div className="card" style={{marginBottom:16}}><b><Sparkles size={15} style={{verticalAlign:'middle',marginRight:6}}/>Approval queue</b><span className="muted" style={{marginLeft:10}}>{pending.length} pending for {workflow.companyName} · emails are never sent automatically</span></div>{pending.length===0?<div className="card" style={{textAlign:'center',padding:40}}><ClipboardCheck size={30} color="#c9a84c"/><p><b>No emails awaiting approval</b></p><p className="muted">{workflow.websiteAuditEnabled?'Analyse a public business website to create the first audit and proposal.':'Find public Dorset prospects to draft the first proposal.'}</p></div>:<div className="grid">{pending.map(d=>{const p=prospectMap.get(d.prospectId);const b=businessMap.get(d.businessId);if(!p||!b)return null;return <div className="card" key={d.id}><div style={{display:'flex',justifyContent:'space-between',gap:15}}><div><b>{decodeHtmlEntities(p.name)}</b><div className="muted">{b.name} · {p.email||'No public email'} · {p.audit?`Audit ${p.score}/100`:`Opportunity ${p.score}/100`}</div></div><span className="badge">Pending approval</span></div><h3 style={{fontSize:14,marginBottom:8}}>{decodeHtmlEntities(d.subject)}</h3><p style={{whiteSpace:'pre-line',fontSize:13,lineHeight:1.6}}>{decodeHtmlEntities(d.body)}</p><div className="muted" style={{marginBottom:12}}>Opportunity: {p.recommendedService} · Confidence {p.confidence}% · {p.reasons.map(decodeHtmlEntities).join(' ')}</div><div style={{display:'flex',gap:8,flexWrap:'wrap'}}><button className="btn" onClick={()=>approve(d.id)} disabled={!p.email}><ClipboardCheck size={14}/> Approve</button><button className="btn secondary" onClick={()=>edit(d)}><Pencil size={14}/> Edit</button><button className="btn secondary" onClick={()=>remove(d.id)}><Trash2 size={14}/> Delete</button></div>{!p.email&&<p style={{color:'#b42318',fontSize:12}}>A public recipient email is required before approval.</p>}</div>})}</div>}{readyToSend.length>0&&<div className="card" style={{marginTop:16}}><b>Approved and ready to send</b>{readyToSend.map(d=><div key={d.id} style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:16,padding:'12px 0',borderBottom:'1px solid #eee'}}><span>{decodeHtmlEntities(prospectMap.get(d.prospectId)?.name||'Prospect')}<span className="muted"> · {decodeHtmlEntities(d.subject)}</span>{d.status==='send_failed'&&<span style={{display:'block',color:'#b42318',fontSize:12}}>Last send failed: {d.lastError||'Please retry.'}</span>}</span><button className="btn" onClick={()=>send(d.id)} disabled={sending!==null}><Send size={14}/> {sending===d.id?'Sending…':d.status==='send_failed'?'Retry send':'Send now'}</button></div>)}</div>}</>;
