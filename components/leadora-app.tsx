...
function OutreachPage({businesses,prospects,setProspects,drafts,setDrafts,outreach,setOutreach}:{businesses:BusinessProfile[];prospects:Prospect[];setProspects:(v:Prospect[])=>void;drafts:Draft[];setDrafts:(v:Draft[])=>void;outreach:Outreach[];setOutreach:(v:Outreach[])=>void}) {
  ...
  async function discoverDigitalProspects() {
    ...
    try {
      const response=await fetch('/api/prospects/discover',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({limit:10,companyId:business.id})}); const result=await response.json(); if(!response.ok)throw new Error(result.error||'Public directory search failed.');
      ...
      for(const [index,item] of candidates.entries()){setProgress(`Analysing ${index+1} of ${candidates.length} prospects…`);try {const auditResponse=await fetch('/api/leads/audit',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({website:item.website})});const audited=await auditResponse.json();if(!auditResponse.ok)throw new Error();const audit=audited.audit as Audit;const opportunity=audit.notes.length?`The audit found ${audit.notes.slice(0,2).join(' and ').toLowerCase()}.`:'The website has a solid foundation and may benefit from a focused growth review.';created.push({id:`${business.id}-${crypto.randomUUID()}`,businessId:business.id,name:audited.businessName||item.name,website:audited.website||item.website,email:audited.contactEmail||item.email,phone:audited.phoneNumber||item.phone,location:item.location,industry:item.industry,contactUrl:audited.contactPageUrl||item.contactUrl,googleMapsUrl:item.googleMapsUrl,score:audit.overallScore,reasons:[opportunity,'Public business contact details were published in an OpenStreetMap listing.'],discoveredAt:new Date().toISOString(),contacted:false,audit,recommendedService:workflow.recommendedService,confidence:Math.max(40,Math.min(95,100-audit.overallScore))});}catch{created.push({id:`${business.id}-${crypto.randomUUID()}`,businessId:business.id,name:item.name,website:item.website,email:item.email,phone:item.phone,location:item.location,industry:item.industry,contactUrl:item.contactUrl,googleMapsUrl:item.googleMapsUrl,score:50,reasons:['Public business contact details were published in an OpenStreetMap listing.','The website audit was unavailable, so this draft makes no audit claim.'],discoveredAt:new Date().toISOString(),contacted:false,recommendedService:workflow.recommendedService,confidence:50});}}
      if(!created.length){setSendError('No new public Dorset prospects were returned. Try again shortly.');return;}
      const newDrafts=created.map(p=>{const note=p.audit?.notes[0]?.toLowerCase();const opening=note?`I came across ${p.name} and took a quick look at its public website. I noticed ${note}.`:`I came across ${p.name} through its public Dorset business listing.`;return {id:`draft-${p.id}`,prospectId:p.id,businessId:business.id,subject:`A practical idea for ${p.name}`,body:withClientContact(`Hi there,\n\n${opening}\n\n${business.name} helps organisations with ${business.services.slice(0,2).join(' and ')}. If useful, I’d be happy to share a few practical, no-obligation ideas that may be relevant.\n\nWould a short call next week be helpful?`,business),callToAction:'Would a short call next week be helpful?',generatedAt:new Date().toISOString(),status:'pending'} satisfies Draft});
  ...
  async function discover() {
    ...
    try {
      ...
      const draft:Draft={id:`draft-${prospect.id}`,prospectId:prospect.id,businessId:business.id,subject:`A practical idea for ${prospect.name}`,body:withClientContact(`Hi there,\n\nI came across ${prospect.name} and took a quick look at its public website. ${evidence}\n\n${business.name} helps organisations with ${business.services.slice(0,2).join(' and ')}. If useful, I’d be happy to share a few practical, no-obligation ideas that relate to what I found.\n\nWould a short call next week be helpful?`,business),callToAction:'Would a short call next week be helpful?',generatedAt:new Date().toISOString(),status:'pending'};
  ...
  return <><Header title={workflow.title} sub={workflow.description} action={<div style={{display:'flex',gap:8,alignItems:'flex-end',flexWrap:'wrap'}}><select aria-label="Outreach company" className="field" style={{margin:0,width:220}} value={businessId} onChange={e=>selectBusiness(e.target.value)}>{businesses.map(b=><option key={b.id} value={b.id}>{b.name}</option>)}</select>{workflowInputs}{businessId==='bryant-digital'&&<button className="btn secondary" onClick={discoverDigitalProspects} disabled={busy}>Find up to 10 Dorset prospects</button>}<button className="btn" onClick={discover} disabled={busy}>{busy?(progress|| (workflow.websiteAuditEnabled?'Analysing…':'Finding…')):workflow.buttonText}</button></div>}/>
