  ...
  function discover() {
    ...
    setTimeout(() => {
      ...
      const found = candidates.filter(row => !existingWebsites.has(row[1])).map((row, index):Prospect => ({
        id: `${business.id}-${Date.now()}-${index}-${Math.random().toString(36).slice(2,8)}`,
        ...
        reasons: [`Located inside ${business.serviceArea}`, 'Public business email found', index%2?'Relevant industry':'Website opportunity identified'],
...
function SettingsPage(){
  const [daily,setDaily]=useState('10'); const [style,setStyle]=useState('Professional and warm');
  function save(){localStorage.setItem('leadora-outreach-settings',JSON.stringify({dailyLimit:Number(daily),style}));}
  return <><Header title="Settings" sub="Manage your workspace, account and security."/><div className="tabs">{['Profile','Team','Billing','Integrations','Preferences','Security'].map((x,i)=><div className={`tab ${i===5?'active':''}`} key={x}>{x}</div>)}</div><div className="grid dashboard-grid"><div className="card"><b>AI outreach preferences</b><label className="form-row" style={{display:'block',fontSize:12}}>Daily prospect limit<input className="field" type="number" value={daily} onChange={e=>setDaily(e.target.value)}/></label><label className="form-row" style={{display:'block',fontSize:12}}>Writing style<select className="field" value={style} onChange={e=>setStyle(e.target.value)}><option>Professional and warm</option><option>Friendly and helpful</option><option>Clear and consultative</option></select></label><label className="form-row" style={{display:'block',fontSize:12}}>Working hours<input className="field" defaultValue="09:00 – 17:00"/></label><button className="btn" onClick={save}>Save preferences</button></div><div className="card"><b>Change Password</b>{['Current Password','New Password','Confirm New Password'].map(x=><label className="form-row" style={{display:'block',fontSize:12}} key={x}>{x}<input className="field" type="password"/></label>)}<button className="btn">Update Password</button></div></div></>}
